...
function SummaryList({
  ...
  return (
    <div>
      <h3 className="mb-1.5 text-[11px] font-medium text-ink">{title}</h3>
      {items.length > 0 ? (
        ...
        <p className="text-[12px] italic text-ink-faint">{emptyText}</p>
      )}
    </div>
  );
}
...
export default function RevisionSummaryPanel({
  ...
  return (
    <section className="rounded-sm border border-border/80 bg-paper-dark/10">
      <div className="border-b border-border/60 px-4 py-3">
        <h2 className="font-serif text-sm font-semibold text-ink">수정 요약</h2>
        <p className="mt-0.5 text-[11px] text-ink-faint">
          교수 검토 기록을 보완하는 간략 요약입니다.
        </p>
      </div>
      ...
      <div className="space-y-4 p-4">
        ...
        <details className="rounded-sm border border-border/60 bg-paper/60">
          <summary className="cursor-pointer px-3 py-2 text-[11px] font-medium text-ink-muted hover:bg-highlight/40">
            상세 반영 내역 보기
          </summary>
          <div className="space-y-4 border-t border-border/60 p-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2 border-t border-border/50 pt-3">
              <div>
                <h3 className="mb-1 text-[11px] font-medium text-ink">
                  교수 추가 지시 반영 여부
                </h3>
